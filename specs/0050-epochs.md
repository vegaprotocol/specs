...
execute (as this would exceed the maximum stake the validator wants). To save ressources, the
...
slightly different rounding rules, or even processors with different float units comming to 
different results. 
My proposal here is a bit of a hack; if t+1 (or f+1 in tendermint speak) blocks in a row get 
rejected due to invalid proposals with anything that could be explained through paramerer 
inconsistencies, the chain goes into a backup mode where parameter changes are only allowd 
