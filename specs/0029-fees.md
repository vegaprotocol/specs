...
This ensure that any trade in the network will require the party to pay a fee, even in the case the trade would require a fee smaller than the smallest unit of the asset.
For example, Ether is 18 decimals (wei), the most smallest unit, non divisible is 1 wei, so if the fee calculation was to be a fraction of a wei (e.g 0.25 wei), wich you cannot represent in this currency, then the vega network would round it up to 1.
