...
The network will need to implement a new command to send through the chain.
	- This command will propage a newly create public key through the network, so it can be use to identify new transaction, and verify signature
	- The public key should be associate to some kind of identifier (e.g:partyID used in orders etc), it could be an address derived from the public key.

The network will also need to verify signature for all transaction.
When to verify them will need to be decided and profile as verifying transaction will be at some cost for the nodes, but ideally we should:
	- Verify a signature for a transaction before it's sent to the chain so we can block a transction to be proposed to the chain if it's an invalid signature
