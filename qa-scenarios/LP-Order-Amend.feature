Feature: Replicate LP getting distressed during continuous trading, check if penalty is implemented correctly
  ...
  #check the volume on the order book
   ...
   # check the requried balances 
    ...
    When the parties place the following orders:
      | party  | market id | side | volume | price | resulting trades | type       | tif     | reference      |
      | party0 | ETH/MAR22 | sell | 20     | 1000  | 0                | TYPE_LIMIT | TIF_GTC | party0-sell-3  |
  ...
  And the insurance pool balance should be "0" for the market "ETH/MAR22"
    ...
    Then the order book should have the following volumes for market "ETH/MAR22":
     ...
     #party0 has reeged lp short order, vol 82 in price 1010, vol 20 at price 1000; and long order vol 105 at price 980
     #maintenance_margin_short = (82+20)*1000*3.5569036=362804
     #maintenance_margin_long =105*1000*0.801225765=84129
     #maintenance_margin = max ( maintenance_margin_long, maintenance_margin_short)=362804
   
     # margin initial level is 362805*1.2 = 435366, hence margin account should be 435366, general account should be 450000-435366 = 14634, and bond account should be 50000
    And the parties should have the following account balances:
      | party  | asset | market id | margin | general  | bond |
