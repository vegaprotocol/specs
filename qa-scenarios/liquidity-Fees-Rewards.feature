Feature: Test LP Fees and Rewards; Should also cover liquidity-fee-setting and equity-like-share calc and total stake; to look into and test: If an equity-like share is small and LP rewards are distributed immediately, then how do we round? (does a small share get rounded up or down, do they all add up?)

Feature: Check what happens when distribution period gets set to a larger value (both in genesis and mid-market)
