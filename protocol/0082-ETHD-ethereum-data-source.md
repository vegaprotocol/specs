...
4. Create a market to use an open oracle data source to settle a market and an ethereum oracle to terminate the market
5. Chain events should only be sent when the filter is matched, this can be verified usign an API and the core/data node events (BUS_EVENT_TYPE_ORACLE_DATA) (<a name="0082-ETHD-037" href="#0082-ETHD-037">0082-ETHD-037</a>)
6. Ethereum oracle data sources should only forward data after a configurable number of confirmations (<a name="0082-ETHD-046" href="#0082-ETHD-046">0082-ETHD-046</a>)
