...
- `sla_parameters` - to support the [LP Performance Metric](./0056-REWA-rewards_overview.md#lp-performance-metric), if the distribution metric is `DISPATCH_METRIC_LP_PERFORMANCE`, SLA parameters must be provided for the network to evaluate each LPs liquidity performance. These parameters are the same as defined in the [liquidity mechanisms specification](./0044-LIME-lp_mechanics.md#market-parameters).
