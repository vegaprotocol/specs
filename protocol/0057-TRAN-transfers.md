...
Here, `fees_paid_this_epoch_in_quantum` are the total trading fees paid by a party (arising from `infrastructure_fee` paid, `maker_fee` paid plus `liquidity_fee` paid, over the epoch and expressed in quantum units).
