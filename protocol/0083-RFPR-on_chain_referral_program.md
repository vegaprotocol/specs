...
- current team `id` if the party is part of a referral team
- current `epochs_in_team`
...
The Teams API should now expose a list of all **teams** (which can be filtered by team `id`) with the following information:
- the teams founding **referrer**
- the teams **referees**
- current `team_running_volume`
