...
- [ ] There is an asset `X` on vega / bridge with withdrawal delay threshold `t1` and delay `d1`. Withdrawal in asset `X` below `t1` has no delay i.e. can be finalised on Ethereum as soon as the withdrawal bundle is received. A withdrawal in asset `X` above `t1` will be rejected by the bridge before time `bundle creation + d1` but can be finalised after `d1` time passes from bundle creation. (<a name="0027-ASSP-019" href="#0027-ASSP-019">0027-ASSP-019</a>)
- [ ] There is an asset `X` on vega / bridge with withdrawal delay threshold `t1` and delay `d1`. An asset update proposal is submitted to change these to `t2` and `d2`; it passes voting and is submitted to Ethereum bridge contract. The new thresholds now apply i.e. withdrawal in asset `X` below `t2` has no delay i.e. can be finalised on Ethereum as soon as the withdrawal bundle is received. A withdrawal in asset `X` above `t2` will be rejected by the bridge before time `bundle creation + d2` but can be finalised after `d2` time passes from bundle creation. (<a name="0027-ASSP-020" href="#0027-ASSP-020">0027-ASSP-020</a>)
