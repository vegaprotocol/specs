...
Each market can have exactly one market as a *sucessor* market. 
...
1. if there are two proposals naming the same parent market then whichever one gets into the pending state first (i.e. passes governance vote) becomes the sucessor of the named parent; the other proposal is cancelled with reason "parent market not available". 

Some markets can be labelled as *perpetual* at the time of proposal; in this case they cannot serve as a parent market. 
