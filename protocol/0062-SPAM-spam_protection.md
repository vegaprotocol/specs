...
As (due to Tendermint constraints) it is currently possible to exceed all thresholds within one block, an attacker can always spam one block; to mitigate this, a attacker that does so is tempoearily banned. For now, all bans are independent, i.e., a ban due to excessive voting only affects further votes.
