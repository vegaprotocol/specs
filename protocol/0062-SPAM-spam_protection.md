...
- Any tokenholder that had more than 50% if its votes post-rejected is banned for max (30 seconds, 1/48 of an epoch) or until the next epocvh starts, and all its votes are immediately rejected. E.g. if the epoch duration is 1 day, then the ban period is 30 minutes. If however the epoch is 10 seconds, then the ban period is 30 seconds (or until the start of the next epoch).
