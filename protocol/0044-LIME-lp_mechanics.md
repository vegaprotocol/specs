...
Their obligation for providing liquidity under SLA is the one from the beginning of current epoch (so in particular for a new LP it's `0`). 
At the beginning of the next epoch (after the rewards / penalties for present LPs have been evaluated) the commitmed amount is noted and the LP is expected to provide sufficient liquidity for the epoch.  
