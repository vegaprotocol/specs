...
- If an LP with a liquidity provision and active orders on a market cancel their provision only (orders remain active), after the cancellation penalty (if it applies at end of epoch) the end of next epoch lp will not accrue any rewards or incur penalty for trades on the market. (<a name="0044-LIME-097" href="#0044-LIME-097">0044-LIME-097</a>)
...
- Boundary values are respected for the network parameters
  ...
  - `market.liquidity.providers.fee.calculationTimeStep` valid values: `>=0`, `<= validators.epoch.length` default value of `60m` (<a name="0044-LIME-100" href="#0044-LIME-100">0044-LIME-100</a>)
