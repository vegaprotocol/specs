...
* If the validator (or, the associated key) does not have sufficient stake (at least the network parameter min_own_take), 
  then the reward is set to zero. The corresponding money is kept by the network, not distributed among the other validators. Note this only applies to the part of the reward going directly to such a validator, its delegators should still receive their rewards.
