...
Once the active LP count in a given market reaches the limit only the LP transactions with commitment amount implying [equity-like-share](0042-LIQF-setting_fees_and_rewarding_lps.md) larger then the lowest equity-like-share already active in that market are accepted. If such a situation occurs the existing LP commitment with equity-like-share which is getting replaced have their commitment cancelled (bond returned to margin account, volume no longer placed on book but positions and orders kept in place). If more then one LP implies the same lowest ELS then one with a lower commitment amount should be cancelled. If commitment amounts are the same then the one which has been in the market for the shortest period of time should be cancelled.\
...
* When the number of liquidity provisions in the market is equal to `limits.markets.maxLPs` an attempt to place an additional liquidity provision with valid commitment amount which implies the equity-like-share that is lower than the lowest one already active in the market results in a rejection. Error message attributes the rejection to the limit. (<a name="0078-NWLI-017" href="#0078-NWLI-017">0078-NWLI-017</a>)
* When market is in continuous trading and `limits.markets.maxLPs` is reached then submitting a new liquidity provision order with commitment amount implying equity-like-share that is higher than the lowest one already active in the market results in cancellation of the LP commitment implying it and the newly submitted LP commitment succeeds and becomes active. (<a name="0078-NWLI-018" href="#0078-NWLI-018">0078-NWLI-018</a>)
* When market is in liquidity monitoring auction and `limits.markets.maxLPs` is reached then submitting a new liquidity provision order with commitment amount implying equity-like-share that is higher than the lowest one already active in the market results in cancellation of the LP commitment implying it and the newly submitted LP commitment succeeds. The amount submitted should be such that the market can now leave the liquidity auction and go back to continuous trading mode.(<a name="0078-NWLI-019" href="#0078-NWLI-019">0078-NWLI-019</a>)
...
* Lowering `limits.markets.maxLPs` below the number number of active liquidity provisions in the market results in no immediate changes to the LP commitments in the markets. However, new LPs will now push out existing LPs from the market provided the equity-like-share implied by their commitments is higher then the lowest one already active in the market. (<a name="0078-NWLI-020" href="#0078-NWLI-020">0078-NWLI-020</a>)
